  ...
  <div class="container">
    ...
    <div class="quantity-statistics">
      ...
      <div class="quantity-item">
        <div class="quantity-detail">
          <div class="quantity-detail-box">
            <div class="quantity-title">宠物总数</div>
            <div class="quantity-border-line"></div>
            <div class="quantity">{{petCount}}</div>
          </div>
        </div>
        <div class="quantity-icon"><img src="../../assets/image/about/icon.png" alt="" /></div>
      </div>
      ...
      <div class="quantity-item">
        <div class="quantity-detail">
          <div class="quantity-detail-box">
            <div class="quantity-title">宠物类型总数</div>
            <div class="quantity-border-line"></div>
            <div class="quantity">{{petTypeCount }}</div>
          </div>
        </div>
        <div class="quantity-icon"><img src="../../assets/image/about/icon.png" alt="" /></div>
      </div> 
      <div class="quantity-item">
        <div class="quantity-detail">
          <div class="quantity-detail-box">
            <div class="quantity-title">规范总数</div>
            <div class="quantity-border-line"></div>
            <div class="quantity">{{ruleCount }}</div>
          </div>
        </div>
        <div class="quantity-icon"><img src="../../assets/image/about/icon.png" alt="" /></div>
      </div> 
    ...
    <div class="information">
      ...
      <div class="article">
        <div class="article-title">通知公告</div>
        <div class="article-list">

          <div v-for="(item, index) in notes" class="article-item" @click="handleArticle('https://opensource.guide/how-to-contribute/')">
            <img class="article-thumb" :src="item.image" alt="" />
            <div class="article-detail">
              ...
              <div class="article-tool">
                <div class="pubdate">{{item.create_time}}</div>
...
export default {
  setup() {
    ...
    const getNotes = async () => {
      try {
        loading.value = true
        notes.value = await noteModel.getNotes()
        loading.value = false
      } catch (error) {
        loading.value = false
        if (error.code === 10020) {
    ...
    const getUserCount = async () =>{
      try {
        userCount.value = await petModel.getUserCount()
      } catch (error) {
      }
    }
    const getPetCount = async () =>{
      try {
        petCount.value = await petModel.getCount()
      } catch (error) {
      }
    }
    const getPetTypeCount = async () =>{
      try {
        petTypeCount.value = await petTypeModel.getCount()
      } catch (error) {
      }
    }
    const getRuleCount = async () =>{
      try {
        ruleCount.value = await ruleModel.getCount()
      } catch (error) {
      }
    }
